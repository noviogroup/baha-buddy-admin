...
import { AlertTriangle, BriefcaseBusiness, Handshake, Link2, MapPinned, Plus, Target, X } from 'lucide-react';
    ...
    <div className="flex flex-col gap-5">
      <div className="bg-white rounded-xl border border-hairline p-5 shadow-card baha-gradient-card">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-body max-w-3xl leading-relaxed">Manage partners, link them to canonical places, and prepare the foundation for leads, campaigns, sponsored placements, and partner reporting.</p>
          </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-hairline p-5 shadow-card">
          ...
          <div className="flex justify-end mt-4"><button onClick={createPartner} disabled={saving} className="px-4 py-2 rounded-lg bg-brand-blue text-white text-sm font-semibold hover:bg-brand-blue-dark disabled:opacity-60">{saving ? 'Saving…' : 'Create Partner'}</button></div>
        </div>
      )}

      <div className="grid grid-cols-4 gap-3">
        <StatCard icon={<BriefcaseBusiness size={18} />} label="Partners" value={s.total || 0} sub={`${s.active || 0} active · ${s.prospect || 0} prospects`} />
        <StatCard icon={<MapPinned size={18} />} label="Linked Places" value={s.linkedPlaces || 0} sub="Connected to canonical places" />
        <StatCard icon={<Target size={18} />} label="Leads" value={s.totalLeads || 0} sub={`${s.convertedLeads || 0} converted`} />
        <StatCard icon={<Handshake size={18} />} label="Campaign Revenue" value={money(s.campaignRevenue || 0)} sub={`${s.campaigns || 0} campaigns`} />
      </div>
