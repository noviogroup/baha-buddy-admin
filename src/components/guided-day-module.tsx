...
type GuidedDayPlan = { id: string; title: string; slug: string; area: string; island: string; status: string; mobility_level: string; budget_level: string; duration_min_minutes: number; duration_max_minutes: number; base_price: number; personalized_price: number; concierge_price: number | null; supports_live_guide: boolean; stop_count?: number; updated_at: string };
...
export function GuidedDayModule() {
  ...
  async function setStatus(planId: string, status: string) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || `Status update failed (${res.status})`);
      setActionMessage(`Guide moved to ${status}.`);
      await reload();
    } catch (err: any) {
      setActionError(err.message || 'Unable to update guide status.');
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {actionError && <div className="rounded-xl border border-status-danger/30 bg-status-danger-bg p-4 text-sm font-semibold text-status-danger">{actionError}</div>}
      {actionMessage && <div className="rounded-xl border border-status-success/30 bg-status-success-bg p-4 text-sm font-semibold text-status-success">{actionMessage}</div>}
