...
export function GuidedDayModule() {
  ...
  async function setStatus(planId: string, status: string) {
    setSavingId(planId);
    try { const res = await apiFetch(`/api/day-plans/${planId}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }); if (!res.ok) throw new Error(`${res.status}`); await reload(); } finally { setSavingId(null); }
  }
