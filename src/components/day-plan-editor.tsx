...
export function DayPlanEditor({ planId }: { planId: string }) {
  ...
  async function loadAll() {
    setLoading(true);
    const [planRes, stopsRes] = await Promise.all([
      apiFetch(`/api/day-plans/${planId}`),
      apiFetch(`/api/day-plans/${planId}/stops`),
    ]);
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="bg-white rounded-xl border border-hairline p-5 shadow-card baha-gradient-card">
        ...
        <h1 className="mt-4 text-2xl font-display font-bold text-ink tracking-tight">Guide Editor</h1>
        <p className="mt-2 text-sm text-body">Edit the guide overview, pricing, timing, stops, and live-guide readiness from one place.</p>
        <div className="mt-4 flex gap-2 text-xs font-bold"><a href="#overview" className="rounded-lg bg-white px-3 py-2 text-brand-blue">Overview</a><a href="#stops" className="rounded-lg bg-white px-3 py-2 text-brand-blue">Stops</a></div>
      </div>
      ...
      <form id="overview" onSubmit={savePlan} className="flex flex-col gap-5">
        ...
        <div className="bg-white rounded-xl border border-hairline p-4 shadow-card flex items-center justify-between gap-4"><label className="flex items-center gap-2 text-sm font-semibold text-body"><input name="supports_live_guide" type="checkbox" defaultChecked={plan.supports_live_guide} /> Supports Live Guide</label><select name="status" defaultValue={plan.status} className="rounded-lg border border-hairline px-3 py-2 text-sm"><option value="draft">Draft</option><option value="review">Review</option><option value="published">Published</option><option value="archived">Archived</option></select></div>
        <div><button disabled={saving} className="rounded-lg bg-brand-blue px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{saving ? 'Saving…' : 'Save guide'}</button></div>
      </form>

      <section id="stops" className="grid gap-5 lg:grid-cols-[1fr_380px]">
        <div className="bg-white rounded-xl border border-hairline shadow-card overflow-hidden"><div className="p-5 border-b border-hairline"><h2 className="text-xl font-display font-bold text-ink">Stops</h2><p className="mt-1 text-sm text-body">These stops power the timeline, route preview, and future live guide.</p></div><div className="divide-y divide-hairline">{stops.map(stop => <div key={stop.id} className="p-4"><div className="flex items-start justify-between gap-4"><div><div className="font-semibold text-ink">{stop.stop_order}. {stop.name}</div><div className="text-xs text-muted">{stop.stop_type} · {stop.suggested_duration_minutes} min · {stop.latitude}, {stop.longitude}</div>{stop.description && <p className="mt-2 text-sm text-body">{stop.description}</p>}{stop.baha_tip && <p className="mt-2 text-xs font-semibold text-brand-blue">Tip: {stop.baha_tip}</p>}</div><Link href={`/day-plans/${planId}/edit#stops`} className="text-xs font-bold text-muted">Edit later</Link></div></div>)}{stops.length === 0 && <div className="p-8 text-center text-sm text-muted">No stops yet. Add the first route stop on the right.</div>}</div></div>

        <form onSubmit={addStop} className="bg-white rounded-xl border border-hairline p-5 shadow-card h-fit"><h3 className="text-lg font-display font-bold text-ink">Add stop</h3><div className="mt-4 grid gap-3"><label className="text-xs font-bold uppercase text-muted">Order<input name="stop_order" type="number" defaultValue={stops.length + 1} className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label><label className="text-xs font-bold uppercase text-muted">Name<input name="name" required className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label><label className="text-xs font-bold uppercase text-muted">Type<input name="stop_type" defaultValue="attraction" className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label><label className="text-xs font-bold uppercase text-muted">Address<input name="address" className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold uppercase text-muted">Latitude<input name="latitude" type="number" step="0.0000001" required className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label><label className="text-xs font-bold uppercase text-muted">Longitude<input name="longitude" type="number" step="0.0000001" required className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label></div><label className="text-xs font-bold uppercase text-muted">Duration minutes<input name="suggested_duration_minutes" type="number" defaultValue={20} className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label><label className="text-xs font-bold uppercase text-muted">Description<textarea name="description" rows={3} className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label><label className="text-xs font-bold uppercase text-muted">Baha Buddy tip<textarea name="baha_tip" rows={2} className="mt-1 w-full rounded-lg border border-hairline px-3 py-2 text-sm normal-case font-normal text-body" /></label><button disabled={addingStop} className="rounded-lg bg-brand-blue px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{addingStop ? 'Adding…' : 'Add stop'}</button></div></form>
      </section>
    </div>
  );
}
