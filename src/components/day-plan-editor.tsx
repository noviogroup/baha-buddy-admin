...
export function DayPlanEditor({ planId }: { planId: string }) {
  ...
  const [stopsRefreshKey, setStopsRefreshKey] = useState(0);
  ...
  async function savePlan(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      title: String(form.get('title') || ''), slug: String(form.get('slug') || ''), short_description: String(form.get('short_description') || ''), full_description: String(form.get('full_description') || ''), island: String(form.get('island') || 'New Providence'), area: String(form.get('area') || 'Nassau'), traveler_types: parseList(String(form.get('traveler_types') || '')), interests: parseList(String(form.get('interests') || '')), duration_min_minutes: Number(form.get('duration_min_minutes') || 180), duration_max_minutes: Number(form.get('duration_max_minutes') || 360), mobility_level: String(form.get('mobility_level') || 'moderate'), budget_level: String(form.get('budget_level') || 'moderate'), base_price: Number(form.get('base_price') || 9.99), personalized_price: Number(form.get('personalized_price') || 19.99), concierge_price: Number(form.get('concierge_price') || 0) || null, default_return_buffer_minutes: Number(form.get('default_return_buffer_minutes') || 90), supports_live_guide: form.get('supports_live_guide') === 'on', status: String(form.get('status') || 'draft'),
    };
  ...
  async function addStop(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      stop_order: Number(form.get('stop_order') || stops.length + 1), name: String(form.get('name') || ''), stop_type: String(form.get('stop_type') || 'attraction'), address: String(form.get('address') || '') || null, latitude: Number(form.get('latitude') || 0), longitude: Number(form.get('longitude') || 0), google_place_id: String(form.get('google_place_id') || '') || null, tripadvisor_location_id: String(form.get('tripadvisor_location_id') || '') || null, suggested_duration_minutes: Number(form.get('suggested_duration_minutes') || 20), description: String(form.get('description') || '') || null, baha_tip: String(form.get('baha_tip') || '') || null, kid_friendly: form.get('kid_friendly') === 'on', metadata: { source, source_id: sourceId || null },
    };
