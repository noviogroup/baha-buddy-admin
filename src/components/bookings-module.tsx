          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-hairline bg-surface/50">
                ...
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-muted tracking-wider uppercase">Gross / Net</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-muted tracking-wider uppercase">Payout</th>
            ...
            <tbody>
              {data.bookings.map(booking => (
                <tr key={booking.id} className={`border-b border-hairline last:border-0 hover:bg-surface/50 ${selected?.id === booking.id ? 'bg-brand-blue-light/60' : ''}`}>
                  <td className="px-4 py-3">
                    <div className="font-mono text-xs text-ink">{booking.id.slice(0, 8)}…</div>
                    <div className="text-[11px] text-muted">{booking.provider_reference || booking.stripe_payment_intent_id || booking.reference_id || 'No external ref'}</div>
                  </td>
                  <td className="px-4 py-3 capitalize text-body">{booking.booking_type || 'unknown'}</td>
                  <td className="px-4 py-3 capitalize text-body">{booking.source_surface || 'unknown'}</td>
                  <td className="px-4 py-3 capitalize text-body">{booking.provider || '—'}{booking.partner_name ? <div className="text-[11px] text-muted">{booking.partner_name}</div> : null}</td>
                  <td className="px-4 py-3"><div className="font-semibold text-ink">{money(booking.gross_booking_value ?? booking.amount, booking.currency || 'USD')}</div><div className="text-[11px] text-muted">Net {money(booking.net_revenue ?? booking.amount, booking.currency || 'USD')}</div></td>
                  <td className="px-4 py-3"><div className="font-semibold text-ink">{money(booking.partner_payout_amount || 0, booking.currency || 'USD')}</div><PayoutBadge status={booking.payout_status} /></td>
                  <td className="px-4 py-3"><div className="flex flex-col items-start gap-1"><StatusBadge status={booking.payment_status || booking.status} /><StatusBadge status={booking.provider_status || 'pending'} /><FailureBadge state={booking.failure_state} /></div></td>
                  <td className="px-4 py-3"><button onClick={() => setSelected(booking)} className="px-2.5 py-1 rounded-md border border-hairline text-[11px] font-semibold text-body hover:bg-surface">Detail</button></td>
                </tr>
              ))}
              {data.bookings.length === 0 && <tr><td colSpan={8} className="px-4 py-12 text-center text-muted">No bookings found yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
